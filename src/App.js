...
function PersonalWebsite({m,onBack,onChat}){
  ...
  return <div style={{minHeight:"100vh",background:"#f8f8f8",fontFamily:"system-ui,sans-serif"}}>
    <div style={{background:`linear-gradient(135deg,${m.color}ee,${m.color}77)`,padding:"28px 20px 24px"}}>
      <div style={{display:"flex",justifyContent:"space-between",marginBottom:16}}>
        <button onClick={onBack} style={{background:"rgba(255,255,255,0.2)",border:"none",borderRadius:20,padding:"7px 16px",color:"#fff",fontSize:13,cursor:"pointer",fontWeight:600}}>← Back</button>
...
function AuthModal({onClose,onLogin}){
  ...
  const [msg,setMsg]=useState("");
  const handle=()=>{
    if(!email||!pass){setMsg("Please fill all fields");return;}
    onLogin({name:name||email.split("@")[0],email,plan:"free"});
    onClose();
  };
  return <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.6)",zIndex:2000,display:"flex",alignItems:"center",justifyContent:"center",padding:20}} onClick={onClose}>
    <div style={{background:"#fff",borderRadius:18,width:"100%",maxWidth:380,padding:28}} onClick={e=>e.stopPropagation()}>
      ...
      <div style={{display:"flex",gap:8,marginBottom:20}}>
        {["login","signup"].map(m=><button key={m} onClick={()=>setMode(m)} style={{flex:1,background:mode===m?"#7F77DD":"#f5f5f5",color:mode===m?"#fff":"#666",border:"none",borderRadius:10,padding:"10px",fontWeight:700,fontSize:14,cursor:"pointer"}}>{m==="login"?"Login":"Sign Up"}</button>)}
      </div>
      ...
      <input value={pass} onChange={e=>setPass(e.target.value)} type="password" placeholder="Password" style={{width:"100%",padding:"12px 14px",borderRadius:10,border:"1.5px solid #ddd",fontSize:14,marginBottom:16,boxSizing:"border-box",outline:"none"}}/>
      {msg&&<div style={{fontSize:13,color:"#E24B4A",marginBottom:12,textAlign:"center"}}>{msg}</div>}
      <button onClick={handle} style={{width:"100%",background:"#7F77DD",color:"#fff",border:"none",borderRadius:10,padding:"13px",fontWeight:700,fontSize:15,cursor:"pointer",marginBottom:10}}>{mode==="login"?"Login →":"Create Account →"}</button>
      <button style={{width:"100%",background:"#fff",color:"#333",border:"1.5px solid #ddd",borderRadius:10,padding:"12px",fontWeight:600,fontSize:14,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",gap:8}}><span>🔴</span> Continue with Google</button>
    </div>
  </div>;
}

function MemberDashboard({user,onClose}){
  ...
  return <div style={{position:"fixed",inset:0,background:"#f8f8f8",zIndex:1500,overflowY:"auto",fontFamily:"system-ui,sans-serif"}}>
    <div style={{maxWidth:1000,margin:"0 auto",padding:"20px 16px 80px"}}>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:20}}>
        ...
        <button onClick={onClose} style={{background:"none",border:"0.5px solid #ddd",borderRadius:20,padding:"6px 16px",cursor:"pointer",fontSize:13,color:"#666"}}>← Back</button>
      </div>
  ...
  return <div style={{position:"fixed",inset:0,background:"#0f172a",zIndex:1500,overflowY:"auto",fontFamily:"system-ui,sans-serif"}}>
    <div style={{maxWidth:1200,margin:"0 auto",padding:"20px 16px 80px"}}>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:24}}>
        ...
        <button onClick={onClose} style={{background:"#1e293b",border:"0.5px solid #334155",borderRadius:20,padding:"7px 16px",cursor:"pointer",fontSize:13,color:"#94a3b8"}}>← Exit</button>
      </div>
...
export default function App(){
  ...
  if(profileView)return <>
    <PersonalWebsite m={profileView} onBack={()=>setProfileView(null)} onChat={m=>{setProfileView(null);setChat(m);}}/>
  ...
  return <div style={{maxWidth:"100%",fontFamily:"system-ui,sans-serif",background:"#f8f8f8",minHeight:"100vh"}}>
    <div style={{background:"#fff",borderBottom:"0.5px solid #eee",padding:"12px 20px",position:"sticky",top:0,zIndex:100}}>
      <div style={{maxWidth:1400,margin:"0 auto"}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:tab==="directory"?12:0}}>
          <button onClick={()=>{setTab("directory");setProfileView(null);}} style={{background:"none",border:"none",cursor:"pointer",textAlign:"left",padding:0}}>
          ...
          <div style={{display:"flex",gap:8,alignItems:"center"}}>
            {user?.email==="admin@topmlmleaders.com"&&<button onClick={()=>setShowAdmin(true)} style={{fontSize:12,background:"#E24B4A18",color:"#E24B4A",border:"0.5px solid #E24B4A44",borderRadius:20,padding:"6px 12px",cursor:"pointer",fontWeight:700}}>⚡ Admin</button>}
            ...
            {user?<button onClick={()=>setShowDashboard(true)} style={{fontSize:12,background:"#7F77DD",color:"#fff",border:"none",borderRadius:20,padding:"7px 14px",cursor:"pointer",fontWeight:700}}>👤 {user.name.split(" ")[0]}</button>
    ...
    <div style={{maxWidth:1400,margin:"0 auto",padding:"16px 20px 90px"}}>
      {tab==="directory"&&<>
        ...
        <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(260px,1fr))",gap:16}}>
          {filtered.map(m=><MemberCard key={m.id} m={m} onView={setProfileView} onChat={setChat} setShareOpen={setShareOpen}/>)}
        </div>
      </>}
      ...
      {tab==="me"&&<div style={{maxWidth:480,margin:"0 auto"}}>
        {user?<div>
          ...
          <div style={{background:"#fff",borderRadius:14,padding:20,marginBottom:14,boxShadow:"0 1px 8px rgba(0,0,0,0.06)"}}>
            ...
            <button onClick={()=>setShowDashboard(true)} style={{width:"100%",background:"#7F77DD",color:"#fff",border:"none",borderRadius:10,padding:"11px",fontWeight:700,fontSize:14,cursor:"pointer",marginBottom:10}}>📊 Open Dashboard</button>
    ...
    {showDashboard&&user&&<MemberDashboard user={user} onClose={()=>setShowDashboard(false)}/>}
